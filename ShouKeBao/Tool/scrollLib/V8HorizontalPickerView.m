...
// @synthesize _
@synthesize _arrayOptions ;

#pragma mark - Init/Dealloc
- (id)initWithFrame:(CGRect)frame withDataSource: (NSArray*) array {
    self = [super initWithFrame:frame];
    if ( self ) {
        self._arrayOptions = array ;
        [ self initSetup ] ;
    }
    return self ;
}
...
- (void)initSetup {
    ...
    self.elementFont    = [ UIFont boldSystemFontOfSize : 14.0f ] ;
    self.selectionPoint = CGPointMake ( self.bounds.size.width/2 , 0 ) ;
    
    // self.pickerView..........
    // add carat or other view to indicate selected element
    UIImageView *indicator = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"indicator"]];
    self.selectionIndicatorView = indicator ;
    /*********************************************************/
...
- (NSInteger)numberOfElementsInHorizontalPickerView:(V8HorizontalPickerView *)picker {
    return [self._arrayOptions count];
}

#pragma mark - HorizontalPickerView Delegate Methods
- (NSString *)horizontalPickerView:(V8HorizontalPickerView *)picker titleForElementAtIndex:(NSInteger)index {
    return [self._arrayOptions objectAtIndex:index];
}

- (NSInteger) horizontalPickerView:(V8HorizontalPickerView *)picker widthForElementAtIndex:(NSInteger)index {
    CGSize constrainedSize = CGSizeMake(MAXFLOAT, MAXFLOAT);
    NSString *text = [self._arrayOptions objectAtIndex:index];
...
- (void) setDataSources:(NSArray*) array{
    self._arrayOptions = [ [ NSArray alloc ] initWithArray : array ] ;
    [ self reloadData ] ;
    // [ self._pickerView layoutIfNeeded ] ;
    // [ self._pickerView layoutSubviews ] ;
}
...
-(void) setDataSourceswithTitle:(NSArray*) array withTitle:(NSString*) title
{
    // self.lableTitle.text = title ;
    self._arrayOptions   = [ [NSArray alloc] initWithArray:array ] ;
    // [ self._pickerView layoutIfNeeded ] ;
